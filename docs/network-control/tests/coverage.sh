...
echo "  • Target: pure helpers (network-types) ≥ ~85%; overall lower because"
echo "    network-info includes wifi_scan/join/power (live wpa/ifconfig/doas)."
echo "  • NOT in unit coverage: freebsd-network-ui.cpp, network-widget.cpp,"
echo "    manager.cpp GTK/NM D-Bus, panel/widgets/network.cpp tray chrome."
echo "  • Residual misses: getifaddrs error path, fork/popen failures, euid==0"
echo "    root branch when tests run as non-root, elevated wifi turn on/off."
echo
